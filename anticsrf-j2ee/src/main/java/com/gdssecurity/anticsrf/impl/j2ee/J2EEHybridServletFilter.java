...
/**
 * Filter providing session based token synchronization and double submit cookie CSRF protection
 */
public class J2EEHybridServletFilter implements Filter {

    private static final Logger logger = Logger.getLogger(J2EEHybridServletFilter.class.getName());
    ...
    public void init(final FilterConfig filterConfig) throws ServletException {
        logger.info("The J2EEHybridServletFilter is initializing");
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        try {
            ...
            String tokenFromUser = ensureCookieAndGetUserToken(request, response);

            if (!getBooleanConfigValue(J2EECustomConfig.MONITOR_MODE.configKey())) {
                boolean isVerifiedRequest = false;

                try {
                    // Verify the request token stored in either the request parameter or header, depending
                    // on the protection strategy.
                    isVerifiedRequest = protectionService
                            .isVerifiedRequest(StringUtils.isBlank(tokenFromUser) ?
                                    new J2EECSRFDSCookieRequestContext(tokenParamName, csrfHeadername) : new J2EECSRFRequestContext(tokenParamName), null);
    ...
    private String ensureCookieAndGetUserToken(HttpServletRequest request, HttpServletResponse response) {
        String requestURL = J2EEHelper.getRequestURL(request);

        //  Create request context objectS to parse HttpServletRequest. Create two in order to be flexible when
        //  deciding which protection strategy to use.
        ...
        switch (tokenRecollectionStrategy) {
            ...
            default:
                ...
                String storedCSRFToken = protectionService.getToken(requestURL);
                if (storedCSRFToken == null || storedCSRFToken.equals("")) {
                    storedCSRFToken = protectionService.hasUrlSpecificConfig(requestURL) ?
                            protectionService.generateResourceToken(requestURL, null) :
                            protectionService.generateToken();

                } else {
                    //  If a token exists in the session, add it to a cookie (double submit cookie synchronization),
                    //  else add it in the request attribute (token synchronization protection).
                    tokenFromUser = request.getParameter(tokenParamName);
                }
    ...
    private void logRequestUrlWithoutToken(HttpServletRequest request) {
        ...
        csrfLogger.info("The J2EEHybridServletFilter is running on URL: " + adjustedURL);
    }
